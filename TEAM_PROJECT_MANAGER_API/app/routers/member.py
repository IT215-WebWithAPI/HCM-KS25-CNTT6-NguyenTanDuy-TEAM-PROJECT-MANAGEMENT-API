...
from app.schemas.project_member_schemas import ProjectMemberCreate, ProjectMemberResponse
...
@member_router.post("", status_code=status.HTTP_201_CREATED, response_model=ResponseCreate)
def create_member(
    req: Request,
    project_id: int = Form(..., description="ID dự án"),
    user_id: int = Form(..., description="Thành viên"),
    role: str = Form(..., description="Chức vụ"),
    db: Session = Depends(get_db),
    current_user: UserModel = Depends(get_current_user)
):
    member_input = ProjectMemberCreate(project_id=project_id, user_id=user_id, role=role)
...
@member_router.delete("/{id}/members/{user_id}", status_code=status.HTTP_200_OK, response_model=ResponseCreate)
...
@member_router.get("/{id}/members", status_code=status.HTTP_200_OK, response_model=ResponseCreate)
def get_members(
    req: Request, 
    id: int, 
    db: Session = Depends(get_db)
    ...
    data_response = [ProjectMemberResponse.model_validate(m) for m in members]
